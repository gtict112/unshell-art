...
int isArt() {
    ...
    __android_log_print(ANDROID_LOG_INFO, TAG, "不是art不能dump");
    return 0;
}
...
void getProcessName(int pid, char *name, int len) {
    ...
    memset(name, 0, len);
    read(fp, name, len);
    __android_log_print(ANDROID_LOG_INFO, TAG, "name %s %d", name,len);
    close(fp);
}
...
void hook() {
    ...
    void *addr = dlsym(handle, "_ZN3art7DexFile10OpenMemoryEPKhjRKNSt3__112basic_stringIcNS3_11char_traitsIcEENS3_9allocatorIcEEEEjPNS_6MemMapEPKNS_10OatDexFileEPS9_");
    if (addr == NULL) {
